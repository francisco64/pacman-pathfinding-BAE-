...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.foodLocation = self.startingGameState.getFood().asList()#self.getFoodLocation()
    
        self.goal =  tuple([1,1] + [1]*len(self.foodLocation))
        
        self.start = self.init_pos + tuple([0]*len(self.foodLocation))
    ...
    def getStartState(self):
        ...
        return self.init_pos + tuple([0]*len(self.foodLocation))
    
    
    def getGoalStates(self):
        goal_states = [coordinates + tuple([1]*len(self.foodLocation)) for  coordinates in self.foodLocation]
        """You code here for Task 3:"""
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                ...
                nextFoodState = list(state[2::])
                
                for index,foodCoordinates in enumerate(self.foodLocation):
                    if foodCoordinates == nextPosition and nextFoodState[index]==0:
                        nextFoodState[index]=1
                    if foodCoordinates == (x,y) and state[2::][index]==0:
                        nextFoodState[index]=1
                    if foodCoordinates == nextPosition and nextFoodState[index]==1:
                        nextFoodState[index]=0
 
                nextState = nextPosition + tuple(nextFoodState) 
                cost = self.costFn(nextState)
                successors.append( ( nextState, action, cost) )
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                ...
                for index,foodCoordinates in enumerate(self.foodLocation):
                    ...
                    #print((x,y))
                    #print(state)
                    if foodCoordinates == (x,y) and state[2::][index]==1:
                        nextFoodState[index]=0
                    if foodCoordinates == nextPosition and nextFoodState[index]==0:
                        nextFoodState[index]=1
...
w=1
def bidirectionalFoodProblemHeuristic(state, problem=None):
    "*** YOUR CODE HERE for Task 3 ***"
    
    r_g=state[2::]
    
    not_r_g = [1-x for x in r_g]
    metric=lambda p1,p2 : sum([abs(x - y) for x,y in zip(p1,p2)]  )
    #metric=lambda p1,p2 : sum([abs(x - y)**2 for x,y in zip(p1,p2)])**(1/2)
    #metric = lambda p1,p2 : mazeDistance(p1[0:2],p2[0:2], problem.startingGameState)
    #H = min([metric(state,food) for food in problem.getGoalStates()])
   
    
    dis = [metric(state,food) for food in problem.foodLocation]
    
    H=sum(i[0] * i[1] for i in zip(not_r_g, dis))*w #+ metric((1,1),state)
    #H=max(dis)
    return H
...
def bidirectionalFoodProblemBackwardsHeuristic(state, problem):
    ...
    #H = min([metric(problem.getStartState(),food) for food in problem.getGoalStates()])
    ...
    dis = [metric(state,food) for food in problem.foodLocation]
    H=sum(i[0] * i[1] for i in zip(r_g, dis))*w# + metric(problem.init_pos,state)
    #H=max(dis)
